...
//				//根据裁判系统 控制弹速
				switch (robot_state.shooter_id1_42mm_speed_limit)
				{
						...
						case 16:
						{
								fric_speed = 6100;//14->4900  16->5900
								break;
						}
						default:
						{
				...
				{
						//拨弹
						if(shoot_control.shoot_rc->rc.ch[4] < 600)  	trigger_motor(0.0f);
						else if(shoot_control.shoot_rc->rc.ch[4] > 600)		trigger_motor(5.5f);
							...
							!BUTTEN_TRIG_PIN && (robot_state.shooter_id1_42mm_cooling_limit - power_heat_data_t.shooter_id1_42mm_cooling_heat >= 100)) 
						{
								...
								shoot_control.bullet_flag = 0;
//						}
//						else if(shoot_control.press_l_time == PRESS_LONG_TIME){
...
//		}
		...
		if(shoot_control.shoot_time < 40)
		{
				switch (robot_state.shooter_id1_42mm_speed_limit)
				{
						...
						case 16:
						{
								trigger_motor(9.0f);//14->4900  16->5900
								break;
						}
						default:
						{
...
//				trigger_motor(0);
//		if(abs(shoot_control.fric_left_motor_measure->speed_rpm)<100 || abs(shoot_control.fric_right_motor_measure->speed_rpm)<100)
//				trigger_motor(0);
...
//		press_l_last_s = shoot_control.press_l;
		...
		if(!last_key_CTRL&&shoot_control.shoot_rc->key.v & KEY_PRESSED_OFFSET_CTRL)
			ctal=!ctal;
		last_key_CTRL = shoot_control.shoot_rc->key.v & KEY_PRESSED_OFFSET_CTRL;
		if(/*robot_state.remain_HP<15 && */ctal == 0)
				trigger_motor(0);
...
static void trigger_motor_turn_back(void)
{
			...
			if(shoot_control.stuck_flag == 1)
			{
					...
					if(shoot_control.reverse_time > 110)
					{
							shoot_control.reverse_time = 0;
							shoot_control.stuck_flag = 0;
					}
