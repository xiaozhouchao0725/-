...
static void gimbal_init(gimbal_control_t *init)
{
    ...
    init->gimbal_pitch_motor.gimbal_motor_mode = init->gimbal_pitch_motor.last_gimbal_motor_mode = GIMBAL_MOTOR_RAW;
		init->gimbal_pitch_motor.offset_ecd = 7000;
		init->gimbal_yaw_motor.offset_ecd = 1350;
    //初始化电机pi
		stm32_pid_yaw_init();
...
static void gimbal_pitch_relative_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
    ...
    //是否超过最大 最小值
    if (gimbal_motor->relative_angle_set > 0.50f)
    {
        gimbal_motor->relative_angle_set = 0.50f;
    }
    else if (gimbal_motor->relative_angle_set < -0.50f)
    {
        gimbal_motor->relative_angle_set = -0.50f;
    }
}
